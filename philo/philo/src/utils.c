...
/*   Updated: 2025/05/14 23:47:17 by peda-cos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static const char	*msg[] = {[THINKING] = "is thinking",
			[EATING] = "is eating", [SLEEPING] = "is sleeping",
			[LEFT_FORK] = "has taken a fork", [RIGTH_FORK] = "has taken a fork",
			[DEAD] = "died"};
...
void	destroy_mutexes(t_Context *ctx)
{
	...
	ctx->running = 0; // Ensure all threads know to exit
	pthread_mutex_unlock(&ctx->running_lock);
	// Give threads time to finish and release locks
	usleep(1000);
	i = 0;
...
void	print_logs(t_State state, t_Context *ctx, t_Philo *philo)
{
	...
	if (state != DEAD)
		...
		printf("\033[0;31m%ld %d %s\n\033[0m", get_time_ms(ctx), philo->id,
			get_state_message(state));
	pthread_mutex_unlock(&ctx->print_logs_lock);
}
